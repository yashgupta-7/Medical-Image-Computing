...
%     for i=1:100
        I_recon=I_noise;
        gamma = prs2(k);
        alpha = (1-prs1(k))/prs1(k);%%0.1*3; %%%%2.9 for high,5.3 for medium, 6.7 for low
        s=0.02;
        cnt=0;
        diff=1;
        while(s>0.0000000000001)
            [prob,grad]=g3(gamma,alpha,I_recon,I_noise);
            objk = [objk sum(sum(prob))];
            I_new=I_recon-s*grad;
            [prob2,grad2]=g3(gamma,alpha,I_new,I_noise);
...
% imshow(Im/255);
% title('original');
% % imwrite(fig,'../results/q2_quad/orig.png');
% % subplot(1,3,2);
% fig = figure();
% imshow(Im_noise/255);
% title('noisy');
% % imwrite(fig,'../results/q2_quad/noisy.png');
% % subplot(1,3,3);
% fig = figure();
% imshow(fin_I/255);
% title('reconstructed');
% % imwrite(fig,'../results/q2_quad/recon.png');
